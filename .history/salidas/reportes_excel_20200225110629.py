...
class ReporteTmXls(TemplateView):
    def get (self, request, *args, **kwargs):
        ...
        for q in query:
            ...
            ws.cell(row=controlador,column=6).value=q.obs
            ...
            ws.cell(row=controlador,column=3).value=id_enc

            query2 = TiempoMuertonDet.objects.filter(id=id_enc)
            ...
            ws.cell(row=controlador,column=3).fill = PatternFill(start_color='66CFCC', end_color='66CFCC', fill_type='solid')
            ws.cell(row=controlador,column=3).font = Font(name='calibri', size=11, bold=True)
            ws.cell(row=controlador,column=3).value=query2.fecha_produccion
